...
Date: 8/10/2026, 8/11/2026, 8/13/2026, 8/17/2026, 8/31/2026
Difficulty: Medium
...
### 五刷 (8/31/2026，补 8/26 站) ❌ `add(idx, e)` 当成了赋值——连续第二轮栽在 API 层

模型层第五轮全对：两容器、填坑、`map.put` 排在 `map.remove` 之前，连 `list.remove(list.size()-1)` 走 `remove(int)` 重载都是对的。错在坑位那一行：

```java
list.add(idx, element);       // ❌ 应为 list.set(idx, element)。add(idx,e) 是插入不是覆盖：
                              //    idx 及其后全部右移一位、size +1 → 该删的 element 根本没被盖掉
list.remove(list.size() - 1); // 删掉的是右移之后的新末尾，长度回到原值，但内容整体错位
```

**根因**：把 `add(idx, e)` 当成了「往 idx 这一格写值」。**List 用两个方法分开表达「插入」和「覆盖」，Map 用同一个 `put` 表达两者**（key 在就覆盖、不在就新增，所以 Map 根本没有 set）——这次是**拿 Map 的写入直觉去用 List 的 API**。四刷「不知道该调哪个方法」和本轮「调了但语义记反」是同一个洞的两种形态：模型层稳住之后，缺口整个转移到了 API 层。

**反例 dry run**：`[a, b, c, d]` remove(b)，idx=1、element=d

| 步骤        | list                                       |
| ----------- | ------------------------------------------ |
| 初始        | `[a, b, c, d]`                             |
| `add(1, d)` | `[a, d, b, c, d]` ← b 被挤到 index 2，没死 |
| `remove(4)` | `[a, d, b, c]`                             |

b 存活，而 map 里 b 的 entry 已被 `map.remove(val)` 删掉 → **静默失同步**：后续 `getRandom()` 可能返回 b，再 `remove(b)` 直接返回 false。正确的 `set(1, d)`：`[a, d, c, d]` → 删末尾 → `[a, d, c]`。另外 `add(idx, e)` 内部是 `System.arraycopy` 搬后半段，O(n)——就算歪打正着也已经违背了本题的全部意义。

> **自检信号**：写任何一行改动 list 的代码前，先口答一句「这一步之后 size 该不该变」。答「不变」就只可能是 `set`，写出 `add` 当场现形。同族于 844 的「else if 挂错宿主」——**心智模型对，选错了承载它的那个语法结构**。

**自测点**：remove 里「末尾值盖到坑位」那一行，为什么必须是 `set(idx, last)` 而不是 `add(idx, last)`？说出 `add` 执行完的那一瞬 list 长什么样、不变量在哪一步被打破。
...
class RandomizedSet {
    ...
    public boolean remove(int val) {
        ...
        list.set(idx, last);                  // 末尾值盖到坑位（set 不是 add，size 不变）
...
| 操作                        | 复杂度    | 为什么                                                                                                                                        |
| --------------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `get(k)` / `containsKey(k)` | 平均 O(1) | `hash(key)` 直接算出 bucket 下标，一步跳过去；碰撞时在桶内找（链表 O(k)，桶内超过 8 个转红黑树 O(log k)）                                     |
| `put(k, v)`                 | 摊还 O(1) | 同样定位 bucket。**key 已存在 → 覆盖 value 并返回旧值；不存在 → 新增返回 null**。装载率超 0.75 触发扩容：容量翻倍 + 全部 element rehash，O(n) |
| `remove(k)`                 | 平均 O(1) | 按 key 定位后摘掉，返回被删的 value；key 不存在返回 null，不报错                                                                              |
| `containsValue(v)`          | O(n)      | value 侧没有索引，只能遍历所有 bucket 逐个比                                                                                                  |
| `size()`                    | O(1)      | 内部维护计数 field，不是现数                                                                                                                  |
| `getOrDefault(k, d)`        | 平均 O(1) | key 不存在时返回 d，省掉「containsKey 再 get」的两次 hash                                                                                     |

**一句话记法：HashMap 的一切能力都来自「key 经 hash 直接算出位置」。** 凡是按 key 的操作全 O(1)，凡是按 value 的操作全退化成 O(n)。

**和 ArrayList 恰好互补**：ArrayList 按下标 O(1)、按值 O(n)；HashMap 按 key O(1)、按 value O(n)。本题要的「按下标随机取」和「按值查存在性」正好一边一个——**两容器模型不是凑出来的技巧，是这两张表拼接的必然**。哪一侧放什么也因此不是随意的：element 必须坐在 key 位才享受得到 O(1) 的 containsKey，index 只能坐 value 位（方向反过来会怎样，见三刷自测点）。

**两个包装类的坑**：`map.get(k)` 返回的是 `Integer`，key 不存在时是 null，直接赋给 `int` 会 NPE——本题因为前面有 `containsKey` 挡着才安全；另外 `Integer` 的缓存只覆盖 -128~127，比较两个 `Integer` 永远别用 `==`。

### 写入语义对照（五刷补，本轮 bug 的正根）

|          | 新增一个                                          | 覆盖已有位置                                       |
| -------- | ------------------------------------------------- | -------------------------------------------------- |
| **List** | `add(e)` 尾部 / `add(i, e)` 中间插入，**size +1** | `set(i, e)`，**size 不变**，返回旧值               |
| **Map**  | `put(k, v)`，key 不存在时，**size +1**            | `put(k, v)`，key 已存在时，**size 不变**，返回旧值 |

**List 把「插入」和「覆盖」拆成两个方法，Map 用同一个 `put` 表达两者**——这就是 Map 没有 `set` 的原因，也是把 Map 的写入直觉带进 List 会翻车的原因。判据永远是 size：**要改一个已存在的位置，size 不该变，那就只能是 `set`。**
...
- **心智模型对 ≠ 落笔对**（二刷 / 五刷）：二刷是多写了一步没人会读的赋值（**动手前先问「这一步的结果还会被人读到吗」**），五刷是选错了承载语义的那个方法。同一句话的两面
- **`add` 永远意味着「多了一个」，改已有位置永远是 `set`**（五刷）：判据是 size 变不变。**别把 Map 的写入直觉带进 List**
- **分清「哪个性质在保护正确性」**（三刷）：紧凑性是 ArrayList 自带的、不需要保护；需要保护的是 map 那本账。找错保护对象 → 会把 O(n) 的代价误判成正确性问题
- **`Random` 当 field 造一次**，别每次调用现 new。（现代 JDK 的无参构造用 `seedUniquifier ^ System.nanoTime()` 做 seed，连续 new **不会**真撞同一序列，那是老版本的坑；但每进一次方法 new 一个对象纯属浪费。多线程场景换 `ThreadLocalRandom.current()`——`Random` 本身线程安全，但多线程抢同一个 seed 会 CAS 空转）
...
- LC844（五刷同族：心智模型正确，但选错了承载它的语法结构——那题是 else if 挂错宿主）

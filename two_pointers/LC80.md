...
7/23 的 invariant 维护（slow 条件推进）没再犯，框架一次写对；卡在初始化。我的代码：

```java
int slow = 0;   // ❌① fast 从 1 起 → index 0 不会被 loop 处理，其 valid 身份必须预先计入 → slow 应为 1；
                //     否则每次写入落早一格，return 值少 1（静默错误）
int count = 1;

for(int fast = 1; fast < nums.length; fast++){
    if(nums[fast] == nums[fast-1]){
        count++;
    }else if(nums[fast] != nums[fast-1]){   // ❌② 冗余 condition（dead code），写 else 即可——坏味道
        count = 1;
    }
    if(count <= 2){
        nums[slow] = nums[fast];
...
**①的根因**：count=1 的注释已经承认「first element is valid」，slow=0 却宣称「valid 区间为空」——两个初值基于矛盾的前提。**规则：slow 初值 = loop 前已预认 valid 的 element 个数 = fast 的起始 index**（27 的 slow=0 对应 fast=0，本题 slow=1 对应 fast=1，配对推出、不用背）。反例 [1,1,2]：nums[0] 的身份从未计数 → return 2 而非 3。

> **自检信号**：初始化 slow/fast/count 后先口头对一句——「loop 不会碰的前缀有几个 element、算不算 valid」，三个初值必须讲同一个故事。

Date: 一刷 7/23/2026, 二刷 7/24/2026, 三刷 7/29/2026

### 三刷 (7/29/2026) ⚠️手生 框架全对，卡在自测点②的口述

15min，一稿二稿的 6 个坑全避掉，且不是碰巧：count 声明在 loop 外、slow 只在 `count <= 2` 里推进、写入与 `slow++` 成对且唯一、`else` 裸写（互补条件降级成注释）。**❌⑤ 那个两稿两犯的 else if 习惯，这次改掉了习惯本身，不是记住了「这里错过」。**

return 的推导第一次从「猜」变成「推」：**用 loop 之前的初始状态锚定 slow 的语义，再把这个语义带到结尾**——「进 loop 前 slow 停在 1，index 0 已计入 → 结尾 slow 表示个数 → 直接 return」。判据的位置从结尾搬到开头，而开头的状态是自己定的、确定无疑，从确定处出发才推得动。26 三刷那个 return 差 1 就是缺这一步。

自测点表现：① ✅ ③ ✅（裂缝打开的时机和后续搬运都答对，只漏了「裂缝永不合拢」）④ ✅。**② 停在现象层**——答「slow 和 fast-1 位置意义不同」，而笔记里的因果是「`nums[slow]` 是待覆盖空位、**值已被算法自己改过、不可信**；`nums[fast-1]` 永不被写入污染」。26 四刷同一自测点也答成「位置不一定相同」，**两天两次同款**：不是记不住，是那条因果链没被调用过。

> **下次② ③ 的口答要点**（不看笔记必须说出）：
> ② 关键词是**污染**与**不可信**，不是「位置不同」——拿输出区当分支依据 = 让算法依赖自己的中间产物（283 的铁律）。另补 count 的精确语义：走到 fast 时该值已连续出现第几次（含自己），题面「至多两次」才直译成 `count <= 2`。
> ③ 补上后果：slow 每轮最多 +1、fast 必 +1 → **裂缝一旦打开永不合拢** → 之后所有写入都是真搬运，代码因此不必区分空转与真搬运。
